-- Odentia Core — restore default Tratamientos for new + zero-treatment clinics
--
-- Bug: the treatments migration (20260831150000_create_treatments.sql)
-- deliberately seeded the existing default catalog into every clinic that
-- existed at that time, but left bootstrap_clinic() untouched on purpose —
-- the intent then was "a new clinic's admin builds her own catalog from
-- Configuración → Tratamientos". In practice this means every clinic
-- created since (e.g. Muelitas7) starts with an empty catalog, so Nueva
-- cita's Tratamiento picker shows only "Sin definir". This migration
-- reverses that specific decision: new clinics should get the same
-- starting catalog clinics always used to have.
--
-- This is purely operational default data for the Cita's planned
-- Tratamiento field — never CUPS/RIPS. RIPS keeps reading exclusively from
-- encounter_diagnoses/encounter_services, untouched here.
--
-- The 8 names below are the exact same canonical list as
-- dashboard/mock-data.ts's TREATMENT_OPTIONS and the original seed in
-- 20260831150000_create_treatments.sql — reused verbatim, not reinvented.

-- ============================================================
-- 1. bootstrap_clinic() — seed the default catalog for every new clinic
-- ============================================================
--
-- create or replace, not drop+create: signature/contract is byte-for-byte
-- identical to 20260826162841_add_clinic_location_coordinates.sql's version
-- (same 22 parameters, same return table) — only a new step 7 is appended
-- to the body. Everything else (auth.uid()-only identity, SECURITY
-- DEFINER, `set search_path = ''`, existing validation, the single
-- transaction across clinics/clinic_locations/clinic_memberships/
-- professional_profiles) is unchanged.
...
begin
  ...
  -- 7. treatments — seed the default catalog every clinic used to get
  -- automatically before this table existed (see the treatments
  -- migration's own seed comment), so Nueva cita's Tratamiento picker is
  -- never empty for a brand-new clinic. v_clinic_id is brand new here, so
  -- there is no existing row to collide with — no ON CONFLICT needed.
  -- "Sin definir" is a UI-level fallback option, not a row in this table
  -- (see real-new-appointment-modal.tsx), so it's unaffected by this seed.
...
-- repeating them here matches this repo's existing convention for every
-- prior create-or-replace of this function.
...
-- ============================================================
-- 2. Backfill — existing clinics with ZERO treatments (e.g. Muelitas7)
-- ============================================================
--
-- Scoped to "this clinic has no treatments row at all" (NOT EXISTS),
-- never a per-name ON CONFLICT DO NOTHING — so a clinic that already built
-- any catalog of its own, custom or not, is left completely untouched.
-- Only a clinic with literally zero rows in public.treatments receives the
-- default catalog. ON CONFLICT DO NOTHING is kept as a defense-in-depth
-- safety net (treatments_clinic_id_name_key), not the primary guard.
--
-- Idempotent: safe to run more than once. After the first run, every
-- affected clinic has at least one treatments row, so the NOT EXISTS
-- check excludes it on any subsequent run — including a second
-- application of this same migration.
insert into public.treatments (clinic_id, name)
select c.id, t.name
