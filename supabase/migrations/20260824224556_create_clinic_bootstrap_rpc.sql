-- Odentia Core — bootstrap_clinic RPC (Paso 3E)
--
-- The controlled, atomic way to create a person's first (or Nth) clinic:
-- clinics + clinic_locations (sede principal) + clinic_memberships
-- (role = clinic_admin) + optionally professional_profiles, in one
-- transaction. This exists precisely because INSERT on all four of those
-- tables is deny-by-default under RLS for `authenticated` (see the
-- previous migration's comments on clinics/clinic_memberships/
-- professional_profiles INSERT) — this function is the one sanctioned
-- path through that wall, not a new permissive policy.
--
-- SECURITY DEFINER is required for exactly that reason: the function must
-- write to RLS-protected tables regardless of who calls it, but it does
-- NOT widen what callers can do — every identity/ownership fact it writes
-- comes from auth.uid(), never from a client-supplied parameter. No new
-- INSERT policy is added anywhere in this migration.

create function public.bootstrap_clinic(
  clinic_name text,
  clinic_slug text,
...
revoke execute on function public.bootstrap_clinic(
  ...
  boolean, uuid, text, text, integer, text
) from public;
...
-- Deliberately NOT granted to anon: bootstrapping a clinic always requires
-- an authenticated identity to attach as clinic_admin.
