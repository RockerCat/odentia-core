...
export function AusenciasAdminSection() {
  const { dentists, soloDentistClinic } = useEffectiveDentists();

  // Auto-selects the first dentist in the list on load (see task scope).
  // Re-derived every render (not just the useState initializer) because
  // soloDentistClinic hydrates one tick after the server-safe first paint
  // (see role-context.tsx) — a plain lazy initializer would freeze on
  // whichever dentist was first before that correction landed.
  const [selectedDentistIdState, setSelectedDentistId] = useState(dentists[0]?.id ?? "");
  const selectedDentistId = dentists.some((dentist) => dentist.id === selectedDentistIdState)
    ? selectedDentistIdState
    : (dentists[0]?.id ?? "");
  ...
  return (
    <div className="rounded-2xl border border-border bg-background p-5 shadow-sm sm:p-6">
      <div className="flex flex-wrap items-start justify-between gap-3">
        ...
        {/* Solo-practitioner clinic (see role-context.tsx): there's only
            ever one dentist (herself), so the CTA lives here directly
            instead of the right column's per-selection header below. */}
        {soloDentistClinic && (
          <button
            type="button"
      ...
      {soloDentistClinic ? (
        // No Profesionales list/selector at all — a one-person clinic has
        // nobody else to pick from (see task scope).
        <div className="mt-4">
          {absences.length === 0 ? (
