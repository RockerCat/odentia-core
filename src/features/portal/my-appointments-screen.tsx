...
// pendingReschedule only stores a day key, not which week it belongs to —
// this resolves it back to a display label by checking the modeled week
// first, then scanning forward the same way findNextAvailableSlot does.
function resolveDayLabel(dayKey: string): string {
  for (let offset = 0; offset <= MAX_RESCHEDULE_WEEKS_AHEAD; offset++) {
    const match = buildWeekDaysForOffset(offset, WEEK_DAYS).find((d) => d.key === dayKey);
    if (match) return `${match.label}, ${match.dateLabel}`;
  }
  return dayKey;
}
...
export function MyAppointmentsScreen() {
  ...
  const nextAppointmentDay = nextAppointment ? WEEK_DAYS.find((d) => d.key === nextAppointment.day) : null;
  ...
  const handleConfirm = () => updateNextAppointment({ status: "confirmed" });
  ...
  return (
    <div className="flex flex-col gap-6">
      {/* Primary CTA only when there's nothing to feature yet — once a
          Próxima cita exists, it's the protagonist and this moves to a
          secondary action after the content instead (see bottom). */}
      {!nextAppointment && (
        ...
        {nextAppointment ? (
          <div className="mt-4 grid grid-cols-1 gap-6 xl:grid-cols-[minmax(0,240px)_1fr_minmax(0,260px)]">
            {/* Profesional — same credential-card language as
                ...
                read-only: no edit affordances for a Patient. */}
            <div className="rounded-xl border border-border bg-[color-mix(in_oklab,var(--primary)_10%,transparent)] p-4 shadow-sm">
              ...
              {/* The clinic's WhatsApp, not the dentist's own — a Patient
                  never sees a professional's private phone/email here. */}
              <a
          ...
          <p className="mt-3 text-sm text-muted-foreground">No tienes citas próximas programadas.</p>
        )}
      </div>
...
// implementation. Opens pre-selected on the current dentist's next open day
...
//
// Changing odontólogo re-runs that same search for the new dentist.
// Submitting never touches the real
// appointment: it only records a `pendingReschedule` slot+dentist (see
// onConfirm/handleRescheduleConfirmed above) and this modal moves to a
// "sent" confirmation phase — approval isn't built yet (no backend).
function RescheduleModal({
  ...
  const initialSlot = findNextAvailableSlot(appointment.dentistId, appointment.id);

  const [selectedDentistId, setSelectedDentistId] = useState(appointment.dentistId);
  ...
  const occupiedTimes = selectedDay ? occupiedTimesFor(selectedDentistId, selectedDay, appointment.id) : new Set<string>();
  ...
  const handlePickDentist = (dentistId: string) => {
    ...
    const next = findNextAvailableSlot(dentistId, appointment.id);
  ...
  const handleSubmit = () => {
    ...
    onConfirm(selectedDay, selectedTime, selectedDentistId);
    setSent(true);
  };
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          {sent ? (
            ...
            <>
              ...
              {/* Odontólogo para la reprogramación — dropdown enriquecido,
                  parte del odontólogo actual por defecto. Se despliega como
                  overlay anclado (mismo patrón que los editores de
                  Fecha/Horario, appointment-detail-modal.tsx) sin ocultar
                  el resto del flujo. */}
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate text-sm font-medium text-foreground">
                      {selectedDentist?.name ?? "Sin asignar"}
                    </p>
                    <p className="truncate text-xs text-muted-foreground">
                      {selectedDentist?.specialty ?? "Odontología general"}
                    </p>
                    <p className="mt-0.5 truncate text-[11px] text-primary">
                      Próx. disponibilidad: {formatNextAvailability(selectedDentistId, appointment.id)}
                    </p>
                  </div>
                  <ChevronIcon
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            ...
                            <p className="mt-0.5 truncate text-[11px] text-primary">
                              Próx. disponibilidad: {formatNextAvailability(d.id, appointment.id)}
                            </p>
                          </div>
              ...
              {/* Navegación semanal + selector de días horizontal, mismo
                  patrón visual/de datos que la Agenda. */}
              <div className="mt-4 flex items-center justify-center gap-1">
                ...
                <span className="min-w-0 px-1 text-center text-sm font-semibold tracking-tight">
                  {currentWeekLabel}
                </span>
                <button
                  type="button"
              ...
              {selectedDay && (
                <div className="mt-4">
                  <p className="text-sm font-medium text-foreground">
                    Horarios disponibles — {selectedDayInfo?.label}
                  </p>
                  {hasAvailableSlots ? (
              ...
              {selectedDay && selectedTime && (
                <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 p-3">
                  ...
                  <p className="mt-0.5 text-xs text-muted-foreground">{selectedDentist?.name ?? dentistName}</p>
                </div>
              )}
            </>
          )}
