...
// Metadata-only edit — título/categoría, never the file (see task scope:
// "No modificar/reemplazar el archivo en esta tarea"). Same modal
// shell/field conventions as UploadDocumentModal/EditAntecedentesModal,
// just without the file input.
export function EditDocumentModal({
  ...
  onClose: () => void;
  onSaved: (document: ClinicalDocumentRecord) => void;
}) {
  const [title, setTitle] = useState(document.title ?? "");
  const [kind, setKind] = useState<ClinicalDocumentKind>(document.kind);
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    const outcome = await updatePatientClinicalDocument({
    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center sm:items-center sm:p-4" onClick={onClose}>
      ...
      >
        <div className="flex shrink-0 items-center justify-between border-b border-border px-5 py-3">
          <p className="text-sm font-semibold">Editar información</p>
          <button
            type="button"
            onClick={onClose}
            aria-label="Cerrar"
        ...
        <div className="flex-1 overflow-y-auto px-5 py-4">
          <div className="flex flex-col gap-3">
            <label className="flex flex-col gap-1 text-sm">
              <span className="text-label-foreground">Título</span>
              <input
                type="text"
                value={title}
