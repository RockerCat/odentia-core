...
type AppShellProps = {
  ...
  heading: ReactNode;
  // Route-level role gate (e.g. /admin → ["superadmin"]) — checked on
  // every render, in both dev and production, so switching role via the
  // dev switcher while already on a gated page kicks the user out
  // immediately instead of just hiding its sidebar link. Omit for pages
  // any authenticated role can see.
  allowedRoles?: Role[];
  children: ReactNode;
};

export function AppShell({ activeNavLabel, heading, allowedRoles, children }: AppShellProps) {
  const authorized = useRouteGuard(allowedRoles);
